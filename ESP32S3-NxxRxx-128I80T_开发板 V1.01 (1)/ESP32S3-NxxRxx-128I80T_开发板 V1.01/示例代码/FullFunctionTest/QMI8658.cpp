...
unsigned char QMI8658_write_reg(unsigned char reg, unsigned char value)
{
	...
	while ((!ret) && (retry++ < 5))
	{
    Wire.beginTransmission(QMI8658I2CAddr);
    Wire.write(reg);
    Wire.write(value);
...
unsigned char QMI8658_read_reg(unsigned char reg, unsigned char *buf, unsigned short len)
{
  ...
  Wire.requestFrom(QMI8658I2CAddr, len, true);
...
{
  int QMI8658_chip_id = 0;
  Wire.beginTransmission(QMI8658I2CAddr);   //开始传输
  Wire.write(QMI8658DeviceIDReg);
  Wire.endTransmission(false);  //发送前面的指令，但不释放总线不发送结束
  Wire.requestFrom(QMI8658I2CAddr, 1 ,true);   //参数为ture请求完数据会发送结束
  QMI8658_chip_id = Wire.read();
